		...
		try:
			if action in self.selection_actions:
				...
				if focus_id == 2050: # listItems
					...
					trakt = chosen_listitem.getProperty('zwpseudo.trakt')
					...
					for item in self.item_list:
						...
						self.selected_items.append(item.getProperty('zwpseudo.trakt'))
				...
				media_type = chosen_listitem.getProperty('zwpseudo.media_type')
				source_trailer = chosen_listitem.getProperty('zwpseudo.trailer')
				if not source_trailer:
					from resources.lib.modules import trailer
					if media_type == 'show':
						source_trailer = trailer.Trailer().worker('show', chosen_listitem.getProperty('zwpseudo.tvshowtitle'), chosen_listitem.getProperty('zwpseudo.year'), None, chosen_listitem.getProperty('zwpseudo.imdb'))
					else:
						source_trailer = trailer.Trailer().worker('movie', chosen_listitem.getProperty('zwpseudo.title'), chosen_listitem.getProperty('zwpseudo.year'), None, chosen_listitem.getProperty('zwpseudo.imdb'))
				...
				if cm_action == 'browseSeries':
					systvshowtitle = quote_plus(chosen_listitem.getProperty('zwpseudo.tvshowtitle'))
					year = chosen_listitem.getProperty('zwpseudo.year')
					imdb = chosen_listitem.getProperty('zwpseudo.imdb')
					tmdb = chosen_listitem.getProperty('zwpseudo.tmdb')
					tvdb = chosen_listitem.getProperty('zwpseudo.tvdb')
					...
					self.close()
					sysart = quote_plus(chosen_listitem.getProperty('zwpseudo.art'))
					self.execute_code('ActivateWindow(Videos,plugin://plugin.video.zwpseudo/?action=seasons&tvshowtitle=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&art=%s,return)' % (
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('zwpseudo.title', item.get('title'))
					if item.get('tvshowtitle'): listitem.setProperty('zwpseudo.media_type', 'show')
					else: listitem.setProperty('zwpseudo.media_type', 'movie')
					listitem.setProperty('zwpseudo.year', str(item.get('year')))
					listitem.setProperty('zwpseudo.isSelected', '')
					listitem.setProperty('zwpseudo.imdb', item.get('imdb'))
					listitem.setProperty('zwpseudo.tmdb', item.get('tmdb'))
					listitem.setProperty('zwpseudo.tvdb', item.get('tvdb'))
					listitem.setProperty('zwpseudo.trakt', item.get('trakt'))
					listitem.setProperty('zwpseudo.status', item.get('status'))
					listitem.setProperty('zwpseudo.rating', str(round(float(item.get('rating', '0')), 1)))
					listitem.setProperty('zwpseudo.trailer', item.get('trailer'))
					listitem.setProperty('zwpseudo.studio', item.get('studio'))
					listitem.setProperty('zwpseudo.genre', item.get('genre', ''))
					listitem.setProperty('zwpseudo.duration', str(item.get('duration')))
					listitem.setProperty('zwpseudo.mpaa', item.get('mpaa') or 'NA')
					listitem.setProperty('zwpseudo.plot', item.get('plot'))
					...
					listitem.setProperty('zwpseudo.poster', poster)
					listitem.setProperty('zwpseudo.clearlogo', clearlogo)
					listitem.setProperty('zwpseudo.art', jsdumps(art))
