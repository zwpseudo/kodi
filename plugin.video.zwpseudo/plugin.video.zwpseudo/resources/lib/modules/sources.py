...
class Sources:
	...
	def play(self, title, year, imdb, tmdb, tvdb, season, episode, tvshowtitle, premiered, meta, select, rescrape=None):
		...
		try:
			control.sleep(200)
			if control.playlist.getposition() == 0 or control.playlist.size() == 1: playerWindow.clearProperty('zwpseudo.preResolved_nextUrl')
			preResolved_nextUrl = playerWindow.getProperty('zwpseudo.preResolved_nextUrl')
	...
	def sourceSelect(self, title, items, uncached_items, meta):
		...
		try:
			...
			if action == 'play_Item' and self.uncached_chosen != True:
				return self.playItem(title, items, chosen_source.getProperty('zwpseudo.source_dict'), self.meta)
			else:
		...
		try:
			...
			for i in range(len(resolve_items)):
				try:
					...
					if homeWindow.getProperty('zwpseudo.source_progress_is_alive') != 'true':
						try: self.progressDialog.close()
						except: pass
						del self.progressDialog
					from resources.lib.modules import player
				...
				try:
					...
					if url:
						...
						if player_hasVideo: # do not setPropery if user stops playback quickly because "onPlayBackStopped" is already called and won't be able to clear it.
							playerWindow.setProperty('zwpseudo.preResolved_nextUrl', url)
...
#			control.notification(title=item_title, message='Removed %s duplicate sources from list' % (len(self.sources) - len(filter)))
	...
	def getConstants(self):
		self.metaProperty = 'plugin.video.zwpseudo.container.meta'
		self.seasonProperty = 'plugin.video.zwpseudo.container.season'
		self.episodeProperty = 'plugin.video.zwpseudo.container.episode'
		self.titleProperty = 'plugin.video.zwpseudo.container.title'
		self.imdbProperty = 'plugin.video.zwpseudo.container.imdb'
		self.tmdbProperty = 'plugin.video.zwpseudo.container.tmdb'
		self.tvdbProperty = 'plugin.video.zwpseudo.container.tvdb'
		self.labelProperty = 'plugin.video.zwpseudo.container.label'
...
#				count += 1
	...
	def rd_cache_chk_list(self, torrent_List, hashList):
		...
		if len(torrent_List) == 0: return
		try:
			if getSetting('realdebrid.check_cache') == 'true':
