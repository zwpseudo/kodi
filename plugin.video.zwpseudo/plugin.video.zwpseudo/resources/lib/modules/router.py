...
def router(params):
	...
	if action is None:
		...
		isUpdate = control.homeWindow.getProperty('zwpseudo.updated')
		if isUpdate == 'true':
			control.execute('RunPlugin(plugin://plugin.video.zwpseudo/?action=tools_cleanSettings)')
			control.homeWindow.clearProperty('zwpseudo.updated')
			from resources.lib.modules import changelog
			changelog.get('zwpseudo')
	...
	####################################################
		...
		if action == 'tools_ShowNews':
		...
		elif action == 'tools_contextzwpseudoSettings':
			control.openSettings('0.0', 'context.zwpseudo')
			...
			elif cleared: control.notification(message='zwpseudo Log File Successfully Cleared')
			else: control.notification(message='Error clearing zwpseudo Log File, see kodi.log for more info')
	...
	####################################################
		...
		if action == 'play_Item':
			...
			for i in items:
				...
				url = 'plugin://plugin.video.zwpseudo/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&select=1' % (
			...
			if rtype == 'movie':
				...
				r = 'plugin://plugin.video.zwpseudo/?action=play_Item'
			elif rtype == 'episode':
				from resources.lib.menus import episodes
				...
				r = 'plugin://plugin.video.zwpseudo/?action=play_Random&rtype=episode'
			elif rtype == 'show':
				from resources.lib.menus import tvshows
				rlist = tvshows.TVshows().get(url, create_directory=False)
				r = 'plugin://plugin.video.zwpseudo/?action=play_Random&rtype=season'
	...
	####################################################
		...
		elif action == 'cache_clearKodiBookmark': # context.zwpseudo action call only
