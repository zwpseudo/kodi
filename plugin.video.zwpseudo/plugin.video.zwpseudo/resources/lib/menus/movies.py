...
#		self.persons_link = 'https://www.imdb.com/search/name/?count=100&name='
	...
	def unfinishedManager(self):
		try:
			...
			del window
			if selected_items:
				refresh = 'plugin.video.zwpseudo' in control.infoLabel('Container.PluginName')
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.zwpseudo/?action=movies&url=%s,return)' % (quote_plus(url)))
...
#		url = self.persons_link + quote_plus(q)
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.zwpseudo/?action=moviePersons&url=%s,return)' % (quote_plus(url)))
...
####-Context Menu and Overlays-####
				...
				cm.append(('[COLOR red][B]zwpseudo Settings[/B][/COLOR]', 'RunPlugin(%s?action=tools_openSettings)' % sysaddon))
