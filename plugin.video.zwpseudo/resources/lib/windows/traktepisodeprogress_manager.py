			...
			if action in self.selection_actions:
				...
				if focus_id == 2060: # listItems
					...
					if chosen_listitem.getProperty('zwpseudo.isSelected') == 'true':
						...
						season = chosen_listitem.getProperty('zwpseudo.season')
						episode = chosen_listitem.getProperty('zwpseudo.episode')
					...
					for item in self.item_list:
						...
						self.selected_items.append({'imdb': item.getProperty('zwpseudo.imdb'), 'tvdb': item.getProperty('zwpseudo.tvdb'),
							'season': item.getProperty('zwpseudo.season'), 'episode': item.getProperty('zwpseudo.episode')})
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('zwpseudo.tvshowtitle', tvshowtitle)
					...
					listitem.setProperty('zwpseudo.premiered', new_date)
					season = str(item.get('season'))
					listitem.setProperty('zwpseudo.season', season)
					episode = str(item.get('episode'))
					listitem.setProperty('zwpseudo.episode', episode)
					label = '%s  -  %sx%s' % (tvshowtitle, season, episode.zfill(2))
					listitem.setProperty('zwpseudo.label', label)
					labelProgress = str(round(float(item['progress']), 1)) + '%'
					listitem.setProperty('zwpseudo.progress', '[' + labelProgress + ']')
					...
					listitem.setProperty('zwpseudo.genre', str(item.get('genre', '')))
