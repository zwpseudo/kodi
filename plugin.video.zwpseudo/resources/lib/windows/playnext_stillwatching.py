...
class StillWatchingXML(BaseDialog):
	...
	def set_properties(self):
		...
		try:
			...
			self.setProperty('zwpseudo.tvshowtitle', self.meta.get('tvshowtitle'))
			self.setProperty('zwpseudo.title', self.meta.get('title'))
			...
			self.setProperty('zwpseudo.episode', str(self.meta.get('episode', '')))
			self.setProperty('zwpseudo.rating', str(self.meta.get('rating', '')))
			self.setProperty('zwpseudo.landscape', self.meta.get('landscape', ''))
			self.setProperty('zwpseudo.fanart', self.meta.get('fanart', ''))
			self.setProperty('zwpseudo.thumb', self.meta.get('thumb', ''))
			next_duration = int(self.meta.get('duration')) if self.meta.get('duration') else ''
			self.setProperty('zwpseudo.duration', str(int(next_duration)))
			endtime = (datetime.now() + timedelta(seconds=next_duration)).strftime('%I:%M %p').lstrip('0') if next_duration else ''
			self.setProperty('zwpseudo.endtime', endtime)
		except:
			from resources.lib.modules import log_utils
			log_utils.error()
