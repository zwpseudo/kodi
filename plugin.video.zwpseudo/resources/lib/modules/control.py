...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('zwpseudo_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('zwpseudo_settings', jsdumps(settings_dict))
		refresh_playAction()
		refresh_libPath()
...
def getzwpseudoVersion():
	return xbmcaddon.Addon('plugin.video.zwpseudo').getAddonInfo('version')
...
def artPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.zwpseudo').getAddonInfo('path'), 'resources', 'media', theme)

def genreIconPath():
	return joinPath(xbmcaddon.Addon('plugin.video.zwpseudo').getAddonInfo('path'), 'resources', 'media', 'genre_media', 'icons')

def genrePosterPath():
	return joinPath(xbmcaddon.Addon('plugin.video.zwpseudo').getAddonInfo('path'), 'resources', 'media', 'genre_media', 'posters')
...
def getColor(n):
	colorChart = ('FF3CB4DC', 'FF5AC8E6', 'FF1E3A5F', 'FF6EC8DC', 'FF8B5CF6', 'FF7C3AED', 'FF4C1D95', 'FF60A5FA', 'FF93C5FD',
						'FF7DD3FC', 'FF38BDF8', 'FF0EA5E9', 'FF0284C7', 'FFE0F2FE', 'FFFFFFFF', 'nocolor')
	if not n: n = '0'
...
def refresh_playAction(): # for zwpseudo global CM play actions
	autoPlay = 'true' if setting('play.mode') == '1' else ''
	homeWindow.setProperty('zwpseudo.autoPlay.enabled', autoPlay)

def refresh_libPath(): # for zwpseudo global CM library actions
	homeWindow.setProperty('zwpseudo.movieLib.path', transPath(setting('library.movie')))
	homeWindow.setProperty('zwpseudo.tvLib.path', transPath(setting('library.tv')))

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear zwpseudo.log if setting to reverse has been changed
	if homeWindow.getProperty('zwpseudo.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('zwpseudo.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://plugin.video.zwpseudo/?action=tools_clearLogFile)')
