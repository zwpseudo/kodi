...
#		if not self.tmdb_key: self.tmdb_key = ''
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.zwpseudo/?action=collections&url=%s,return)' % (quote_plus(url)))
...
#################################
	...
	def movieDirectory(self, items, next=True):
		...
		sysaddon, syshandle = 'plugin://plugin.video.zwpseudo/', int(argv[1])
...
####-Context Menu and Overlays-####
				...
				cm.append(('[COLOR red]zwpseudo Settings[/COLOR]', 'RunPlugin(%s?action=tools_openSettings)' % sysaddon))
####################################
				...
				if is_widget: item.setProperty('iszwpseudo_widget', 'true')
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
	...
	def addDirectoryItem(self, name, action, poster, icon, fanart=None, context=None, queue=False):
		try:
			...
			url = 'plugin://plugin.video.zwpseudo/?action=%s' % action
			cm = []
